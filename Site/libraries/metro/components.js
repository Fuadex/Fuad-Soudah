...
prx.types.metro_button = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR = cR + '<span data-editableproperty="text">' + prx.componentsHelper.getProp(item.text,'text') + '</span>';
		cR = cR + '</div>';
		cR = cR + '</div>';
		cR = cR + '</div>';
		return cR;
...
prx.types.metro_badge = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		if(prx.componentsHelper.getProp(item.badgeType,'other') == 'number') {
			...
			cR += '<span data-editableproperty="badgeNumber">'+prx.componentsHelper.getProp(item.badgeNumber,'text')+'</span>';
			cR += '</div>';
		}
		cR += '</div>';
		cR += '</div>';
...
prx.types.metro_tile_user = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="metro-tile-user-title"><span data-editableproperty="username">'+prx.componentsHelper.getProp(item.username,'text')+'</span></div>'
		cR += '<div class="metro-tile-user-text"><span data-editableproperty="surname">'+prx.componentsHelper.getProp(item.surname,'text')+'</span></div>'
		cR += '</div>';
		cR += '</div>';
		return cR;
	}
...
prx.types.metro_filters = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.buttons, function(i,elm){
			...
			cR += '<label class="textColor '+((prx.componentsHelper.getProp(item.selected,'num-other') == i) ? 'liveUpdate-activeTextColor' : '')+'" for="'+_id+'-radio-'+i+'"><span  data-editableproperty="text" data-dynamic-property-index="'+i+'">'+prx.componentsHelper.getProp(elm.text,'text')+'</span></label>'
...
prx.types.metro_dropdown_withselection = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		var _dropdownprops = prx.componentsHelper.getProp(item.dropdownTextProperties,'props-text');
...
prx.types.metro_dialog = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cText += '<div class="dialog-title" data-editableproperty="title">'+prx.componentsHelper.getProp(item.title,'text')+'</span></div>';
		...
		$.each(item.buttons, function(i,elm){
			...
			cButtons += '<span data-editableproperty="text" data-dynamic-property-index="'+i+'">'+ prx.componentsHelper.getProp(elm.text,'text') + '</span>'
			cButtons += '</div>'
		});
		cButtons += '</div>';
...
prx.types.metro_toast_mini = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<span class="toast-title"> <span data-editableproperty="title">'+prx.componentsHelper.getProp(item.title,'text')+'</span></span>';
		cR += '<span class="toast-text"> <span data-editableproperty="text">'+prx.componentsHelper.getProp(item.text,'text')+'</span></span>';
		cR += '</div>';
		cR += '</div>';
		return cR;
	}
...
prx.types.metro_toast = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="toast-title" data-editableproperty="title">'+prx.componentsHelper.getProp(item.title,'text')+'</div>';
		cR += '<div class="toast-text"><span data-editableproperty="text">'+prx.componentsHelper.getProp(item.text,'text')+'</span></div>';
		cR += '</div>';
		cR += '</div>';
		return cR;
	}
...
prx.types.metro_tile_icon = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		if(prx.componentsHelper.getProp(item.brandingType,'other') == 'text') {
			cR += '<span data-editableproperty="brandingText">'+prx.componentsHelper.getProp(item.brandingText,'text')+'</span>';
		}
		cR += '</div>';
...
prx.types.metro_tile_text = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="metro-tile-text-title"><span data-editableproperty="title">'+prx.componentsHelper.getProp(item.title,'text')+'</span></div>'
...
prx.types.metro_select = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += (typeof(item.buttons[item.selected]) != "undefined") ? prx.componentsHelper.getProp(item.buttons[item.selected].text,'other') : ((typeof(item.buttons[0]) != "undefined") ? prx.componentsHelper.getProp(item.buttons[0].text,'other') : '');
		cR += '</span>';
		...
		$.each(item.buttons, function(i,elm){
			...
			cR += '<div class="error-msg"><span data-editableproperty="text" data-dynamic-property-index="'+i+'">'+prx.componentsHelper.getProp(elm.text,'text')+'</span></div>'
	...
	,afterDisplay: function(item,containerid,pageid,symbol) {
		var _id = (!containerid) ? item.id : containerid+'-'+item.id;
		if(!prx.editor) {
			$('#'+_id+' .dropdown-trigger').hammer().on('tap',function(){
				$('#'+_id+' .metro-dropdown-contextmenu').toggle();
			});
