...
function spineWidth(item) {
  ...
  if (item.medium === 'TV' || item.medium === 'Animated Series') return 26;
  return 22;
}
...
// ─────────── Shelf row ───────────
function ShelfRow({ medium, items, idx, mode, mixSeed, onOpenItem, justPickedId, pickedSet }) {
  ...
  const ordered = React.useMemo(() => {
    const arr = items.map((it, i) => ({ ...it, _rank: i }));
  ...
  return (
    <section className={`shelf${hoverIdx >= 0 ? ' is-focused' : ''}${isPicking ? ' is-picking' : ''}`}>
...
function Reader({ item, onClose, onJump }) {
  const { ITEMS, MEDIA_SHORT } = window.CULTURE;
      ...
      <div className="reader" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="reader-body">
          ...
          {item.note
            ...
            : <blockquote className="reader-quote empty">A note for this one is on the to-write list.</blockquote>}
...
// ─────────── App ───────────
function App() {
  const { ITEMS, MEDIA, PICKABLE_IDS } = window.CULTURE;
